...
def test_upsert_paper_records_rule_evidence_and_best_level(tmp_path):
    connection, repository = make_repository(tmp_path)
    try:
        match = repository.upsert_paper(
            {
                "title": "A genOway mice paper",
                "doi": "10.1/rule-evidence",
                "abstract": "Humanized mice were generated by genOway for this study.",
            }
        )

        evidence = connection.execute(
            "SELECT evidence_type, evidence_level FROM paper_evidence WHERE paper_id = ?",
            (match.paper_id,),
        ).fetchall()
        paper = repository.get_paper(match.paper_id)

        assert ("supplier_usage", "A") in [(row[0], row[1]) for row in evidence]
        assert paper["evidence_level"] == "A"
    finally:
        connection.close()
